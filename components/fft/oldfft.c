...
// ------------------- State -------------------
static float   g_hop_buf[HOP_SAMPLES];
static size_t  g_hop_fill=0;
static float   g_fft_buf[FFT_SIZE];
static size_t  g_fft_fill=0;
static float   g_window[FFT_SIZE];
static float   g_fft_re[FFT_SIZE];
static float   g_fft_im[FFT_SIZE];
static float   g_prev_logmag[FFT_SIZE/2+1];
...
static float g_fund_hist[BPM_AVG_FRAMES];
static int   g_fund_hist_pos   = 0;
static int   g_fund_hist_count = 0;
static float g_phase_curve_cache[BPM_MAX - BPM_MIN + 1][PHASE_SLOTS];
static uint8_t g_phase_curve_count[BPM_MAX - BPM_MIN + 1];
...
static void comb_bpm_from_novelty(const float *nov, int N, float frameRate, float *bpmSpec, float *bpmPhase){
    ...
    for (int idx = 0; idx < bpmCount; ++idx){
        ...
        memcpy(g_phase_curve_cache[idx], phaseSlots, sizeof(phaseSlots));
        g_phase_curve_count[idx] = PHASE_SLOTS;
...
static void update_bpm_from_novelty(void){
    ...
    static float nov_history[NOV_RING_FRAMES];
    ...
    static float bpmSpec[BPM_MAX - BPM_MIN + 1];
    static float bpmPhase[BPM_MAX - BPM_MIN + 1];
    static float bpmSpecBC[BPM_MAX - BPM_MIN + 1];
    ...
    int fundIdx = (int)lroundf(fund_avg - (float)BPM_MIN);
    if (fundIdx < 0) fundIdx = 0;
    if (fundIdx >= BPM_MAX - BPM_MIN + 1) fundIdx = BPM_MAX - BPM_MIN;
    int dispCount = g_phase_curve_count[fundIdx];
    fft_render_update_phase_curve((dispCount > 0) ? g_phase_curve_cache[fundIdx] : NULL,
                                  dispCount);
}
...
esp_err_t fft_visualizer_start(void){
    ...
    BaseType_t ok = xTaskCreatePinnedToCore(fft_task, "fft_vis", 12288, NULL, 5, &s_task, tskNO_AFFINITY);
